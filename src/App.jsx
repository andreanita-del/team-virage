...
export default function App() {
  ...
  const [teamMembers, setTeamMembers] = useState(teamMembersFallback)
  const [events, setEvents] = useState(eventsFallback)
  const [stock, setStock] = useState(initialStockFallback)
  const [cashflow, setCashflow] = useState(initialCashflowFallback)
  ...
  async function loadAll(){
    const [tm, ev, st, cf, cp, re, ch] = await Promise.all([
      supabase.from('team_members').select('*').order('id'),
      supabase.from('events').select('*').order('id'),
      supabase.from('stock').select('*').order('id'),
      supabase.from('cashflow').select('*').order('id'),
      supabase.from('compras').select('*').order('id'),
      supabase.from('reuniones').select('*').order('id'),
      supabase.from('checklist').select('*').order('id'),
    ])
    if(tm.data && tm.data.length) setTeamMembers(tm.data)
    if(ev.data && ev.data.length) setEvents(ev.data)
    if(st.data && st.data.length) setStock(st.data)
    if(cf.data && cf.data.length) setCashflow(cf.data)
    if(cp.data) setCompras(cp.data.map(c => ({ ...c, necesitaAprobacion: c.necesita_aprobacion, aprobadoPor: c.aprobado_por })))
    if(re.data) setReuniones(re.data)
  ...
  const addCompra = async () => {
    ...
    const payload = {
      concepto: newCompra.concepto,
      importe: parseFloat(newCompra.importe),
      proveedor: newCompra.proveedor,
      evento: newCompra.evento,
      solicitante: newCompra.solicitante,
      estado: 'pendiente',
      necesita_aprobacion: necesitaAprobacion,
      aprobado_por: necesitaAprobacion ? 'Pendiente (Philippe/Gabi/Laura C)' : 'Auto-aprobado'
    }
    const { data, error } = await supabase.from('compras').insert(payload).select().single()
    if (!error && data) {
      setCompras([...compras, { ...data, necesitaAprobacion: data.necesita_aprobacion, aprobadoPor: data.aprobado_por }])
    }
    setNewCompra({ concepto: '', importe: '', proveedor: '', evento: 'Imola', solicitante: '', estado: 'pendiente' })
  }

  const aprobarCompra = async (id) => {
    await supabase.from('compras').update({ estado: 'aprobada' }).eq('id', id)
    setCompras(compras.map(c => c.id === id ? { ...c, estado: 'aprobada' } : c))
  }

  const rechazarCompra = async (id) => {
    await supabase.from('compras').update({ estado: 'rechazada' }).eq('id', id)
    setCompras(compras.map(c => c.id === id ? { ...c, estado: 'rechazada' } : c))
  }

  const addReunion = async () => {
    if (!newReunion.titulo || !newReunion.fecha) return
    const { data, error } = await supabase.from('reuniones').insert(newReunion).select().single()
    if (!error && data) setReuniones([...reuniones, data])
    setNewReunion({ titulo: '', fecha: '', participantes: '', notas: '' })
  }

  const addChecklist = async () => {
    if (!newChecklist.tarea) return
    const { data, error } = await supabase.from('checklist').insert({ ...newChecklist, completada: false }).select().single()
    if (!error && data) setChecklist([...checklist, data])
    setNewChecklist({ tarea: '', evento: 'Imola', responsable: '', prioridad: 'media' })
  }

  const toggleChecklist = async (id) => {
    const item = checklist.find(c => c.id === id)
    await supabase.from('checklist').update({ completada: !item.completada }).eq('id', id)
  ...
  const addCashflow = async () => {
    ...
    const payload = { ...newCashflow, importe: parseFloat(newCashflow.importe) }
    const { data, error } = await supabase.from('cashflow').insert(payload).select().single()
    if (!error && data) setCashflow([...cashflow, data])
  ...
  return (
    <div style={styles.app}>
      ...
      <div style={styles.content}>
        {loading ? (
          <div style={{ color: '#aaa', textAlign: 'center', padding: '40px' }}>Cargando datos...</div>
        ) : renderContent()}
      </div>
    </div>
  )
